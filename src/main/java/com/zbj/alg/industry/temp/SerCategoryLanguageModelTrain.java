...
public class SerCategoryLanguageModelTrain {
	private static final Logger logger = LoggerFactory.getLogger(SerCategoryLanguageModelTrain.class);
	...
	private static Map<String, Double> predict(String title) throws IOException {
		...
		Result words = ToAnalysis.parse(title,UserDefineLibrary.FOREST,zbjdic);
		Result Words = IndexAnalysis.parse(title,UserDefineLibrary.FOREST,serdic);
	...
	private static void constructSerModel(String corpusModelPath) throws IOException {
        ...
        while((line = br.readLine()) != null){
			...
			if(seg.length ==10 ){
				...
				List<Term> tag = new ArrayList<Term>();
				for(int m=0;m<words.size()-1;m++){
//					if(words.get(m).getNatureStr().equals("w")
//							|| words.get(m).getNatureStr().equals("m")
//							|| !words.get(m).toString().contains("/")){
//						continue;
//					}
